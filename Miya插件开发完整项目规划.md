...
- Phase C（跨软件“像人一样”桌控统一动作引擎）：已落地增强版。`desktop.action.execute` 已增强 `window/text/selector/coordinates` 目标解析与 `assert(text)`，修复 `input_mutex` 多步误判与失败步骤定位；新增 `desktop.automation.kpi.get`、`desktop.replay.skills.list` 观测接口。
