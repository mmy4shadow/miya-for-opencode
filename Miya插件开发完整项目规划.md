...
- 未完成项（保持 `进行中`，避免虚报）：  
  - L2 OCR 文本定位尚未形成“独立前置定位器”能力，当前仍以证据校验和降级控制为主。  
  - L3 SoM+VLM 的“编号问答”已预留协议预算，但默认零调用；真实生产 VLM 选号器仍待后端命令与 ROI 管线联调。  
...
| QQ/微信桌面外发主链路（含证据包） | 进行中（VAB首版已落地） | P0 | 2-3周 | desktop_control + outbound_send + Arch Advisor | 已落地结构化 action_plan 协议、L0-L3 路由骨架、SendInput 拟人化执行与 KPI 计量；L2 OCR 前置定位与 L3 VLM 选号器仍在联调（`miya-src/src/channels/service.ts`, `miya-src/src/channel/outbound/shared.ts`, `miya-src/src/channel/outbound/vision-action-bridge.ts`） |
...
- 目标 6（进行中，2026-02-16 启动）：桌控 Vision-Action Bridge 协议升级已落地首版（intent+screen_state/action_plan、L0-L3 路由骨架、SendInput 执行与 KPI 计量），L2/L3 的 OCR 前置定位与 VLM 编号选择器待联调收口。
- 量化验收 KPI（冻结）：
