...
**待确认问题（影响方案取舍）**：
- 你目前使用的是哪个 QQ/微信 客户端（版本/语言/是否多开/是否开启了“简洁模式”等）？UIA 可用性在不同版本差异很大。
- “本地快眼”主要识别哪些关键控件清单（例如：搜索框、会话条目、发送按钮、附件入口、回执标识）？需要你确认优先级，避免模型范围过大导致误检。
- 中文输入法策略：你希望“全程不粘贴”还是“允许小段粘贴作为兜底”？如果全程逐字输入，是否接受在证据包里记录输入法状态/候选框截图（可脱敏）用于排错？
