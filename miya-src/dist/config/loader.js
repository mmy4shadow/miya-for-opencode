...
const PROMPT_DIR_NAME = 'miya';
const CONFIG_BASE_NAME = 'miya';
/**
...
function loadConfigFromPath(configPath) {
    try {
        ...
        if (error instanceof Error &&
            'code' in error &&
            error.code !== 'ENOENT') {
            console.warn(`[miya] Error reading config from ${configPath}:`, error.message);
        }
        return null;
    }
}
 ...
 */
export function loadPluginConfig(directory) {
    const userConfigBase = path.join(getUserConfigDir(), 'opencode', CONFIG_BASE_NAME);
    ...
    if (config.preset) {
        ...
        if (preset) {
            ...
            const availablePresets = config.presets
                ...
                : 'none';
            console.warn(`[miya] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`);
        }
    }
    return config;
}
...
export function loadAgentPrompt(agentName) {
    ...
    const result = {};
    const promptsDir = path.join(promptsRoot, PROMPT_DIR_NAME);
    ...
    if (fs.existsSync(promptPath)) {
        try {
            ...
            console.warn(`[miya] Error reading prompt file ${promptPath}:`, error instanceof Error ? error.message : String(error));
        }
    }
    ...
    if (fs.existsSync(appendPromptPath)) {
        try {
            ...
            console.warn(`[miya] Error reading append prompt file ${appendPromptPath}:`, error instanceof Error ? error.message : String(error));
        }
    }
    return result;
}
