...
export type PlanBundleStage = 'plan' | 'approval' | 'execution' | 'rollback' | 'audit' | 'finalize';
export type PlanBundleMode = 'work' | 'chat' | 'mixed' | 'subagent';
export type PlanBundleRiskTier = 'LIGHT' | 'STANDARD' | 'THOROUGH';
export type PlanBundleLifecycleState = 'draft' | 'proposed' | 'approved' | 'executing' | 'verifying' | 'done' | 'failed' | 'postmortem';
...
export interface PlanBundleV1 {
    ...
    createdAt: string;
    updatedAt: string;
    status: 'draft' | 'pending_approval' | 'approved' | 'running' | 'completed' | 'failed' | 'rolled_back';
    plan: AutopilotPlan;
