...
import { extractChannel, findPluginEntry, getCachedVersion, getLatestVersion, getLocalDevVersion, updatePinnedVersion, } from './checker';
    ...
    return {
        event: ({ event }) => {
            ...
            setTimeout(async () => {
                ...
                if (localDevVersion) {
                    if (showStartupToast) {
                        showToast(ctx, `Miya ${displayVersion} (dev)`, 'Running in local development mode.', 'info');
                    }
                    log('[auto-update-checker] Local development mode');
                    return;
                }
                if (showStartupToast) {
                    showToast(ctx, `Miya ${displayVersion ?? 'unknown'}`, 'miya is active.', 'info');
                }
...
async function runBackgroundUpdateCheck(ctx, autoUpdate) {
    ...
    if (!autoUpdate) {
        showToast(ctx, `Miya ${latestVersion}`, `v${latestVersion} available. Restart to apply.`, 'info', 8000);
    ...
    if (installSuccess) {
        showToast(ctx, 'Miya Updated!', `v${currentVersion} → v${latestVersion}\nRestart OpenCode to apply.`, 'success', 8000);
...
async function runBunInstallSafe(ctx) {
    try {
        ...
        const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve('timeout'), 60_000));
