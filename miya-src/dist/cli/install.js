...
import { addAntigravityPlugin, addChutesProvider, addGoogleProvider, addPluginToOpenCodeConfig, buildDynamicModelPlan, detectCurrentConfig, disableDefaultAgents, discoverModelCatalog, discoverOpenCodeFreeModels, discoverProviderFreeModels, generateLiteConfig, getOpenCodeVersion, isOpenCodeInstalled, pickBestCodingChutesModel, pickBestCodingOpenCodeModel, pickSupportChutesModel, pickSupportOpenCodeModel, writeLiteConfig, } from './config-manager';
...
function formatConfigSummary(config) {
    ...
    lines.push(`  ${config.hasAnthropic ? SYMBOLS.check : `${DIM}○${RESET}`} Anthropic`);
    lines.push(`  ${config.hasCopilot ? SYMBOLS.check : `${DIM}○${RESET}`} GitHub Copilot`);
    lines.push(`  ${config.hasZaiPlan ? SYMBOLS.check : `${DIM}○${RESET}`} ZAI Coding Plan`);
    lines.push(`  ${config.hasAntigravity ? SYMBOLS.check : `${DIM}○${RESET}`} Antigravity (Google)`);
    lines.push(`  ${config.hasChutes ? SYMBOLS.check : `${DIM}○${RESET}`} Chutes`);
    lines.push(`  ${SYMBOLS.check} Opencode Zen`);
    if (config.useOpenCodeFreeModels && config.selectedOpenCodePrimaryModel) {
        lines.push(`  ${SYMBOLS.check} OpenCode Free Primary: ${BLUE}${config.selectedOpenCodePrimaryModel}${RESET}`);
    }
    if (config.useOpenCodeFreeModels && config.selectedOpenCodeSecondaryModel) {
        lines.push(`  ${SYMBOLS.check} OpenCode Free Support: ${BLUE}${config.selectedOpenCodeSecondaryModel}${RESET}`);
    }
    if (config.hasChutes && config.selectedChutesPrimaryModel) {
        lines.push(`  ${SYMBOLS.check} Chutes Primary: ${BLUE}${config.selectedChutesPrimaryModel}${RESET}`);
    }
    if (config.hasChutes && config.selectedChutesSecondaryModel) {
        lines.push(`  ${SYMBOLS.check} Chutes Support: ${BLUE}${config.selectedChutesSecondaryModel}${RESET}`);
    }
...
function printAgentModels(config) {
    ...
    for (const [agent, info] of Object.entries(agents)) {
        ...
        const skillsStr = info.skills.length > 0
...
function argsToConfig(args) {
    return {
        ...
        preferredOpenCodeModel: args.opencodeFreeModel && args.opencodeFreeModel !== 'auto'
...
async function askModelSelection(rl, models, defaultModel, prompt) {
    ...
    const answer = (await rl.question(`${BLUE}${prompt}${RESET} ${DIM}[default: ${defaultIndex + 1}]${RESET}: `))
        .trim()
        .toLowerCase();
    if (!answer)
        return defaultModel;
    ...
    try {
        ...
        if (useOpenCodeFree === 'yes') {
            ...
            if (discovery.models.length === 0) {
                ...
                const recommendedPrimary = pickBestCodingOpenCodeModel(discovery.models)?.model ??
        ...
        if (chutes === 'yes') {
            ...
            if (discovery.models.length === 0) {
                ...
                const recommendedPrimary = pickBestCodingChutesModel(discovery.models)?.model ??
                    discovery.models[0]?.model;
                if (recommendedPrimary) {
                ...
                if (selectedChutesPrimaryModel) {
                    const recommendedSecondary = pickSupportChutesModel(discovery.models, selectedChutesPrimaryModel)
...
async function runInstall(config) {
    ...
    const hasAnyEnabledProvider = resolvedConfig.hasKimi ||
    ...
    if (resolvedConfig.hasAntigravity)
        totalSteps += 2; // antigravity plugin + google provider
    if (resolvedConfig.hasChutes)
        totalSteps += 1; // chutes provider
    if (hasAnyEnabledProvider)
        totalSteps += 1; // dynamic model resolution
    if (resolvedConfig.installSkills)
        totalSteps += 1; // skills installation
    if (resolvedConfig.installCustomSkills)
        totalSteps += 1; // custom skills installation
    let step = 1;
    ...
    if (resolvedConfig.useOpenCodeFreeModels &&
        ...
        if (discovery.models.length === 0) {
            ...
            const selectedPrimary = resolvedConfig.preferredOpenCodeModel &&
                discovery.models.some((model) => model.model === resolvedConfig.preferredOpenCodeModel)
            ...
            resolvedConfig.selectedOpenCodeSecondaryModel =
                resolvedConfig.selectedOpenCodeSecondaryModel ??
                    pickSupportOpenCodeModel(discovery.models, resolvedConfig.selectedOpenCodePrimaryModel)?.model ??
                    resolvedConfig.selectedOpenCodePrimaryModel;
            printSuccess(`OpenCode free models ready (${discovery.models.length} models found)`);
        }
    }
    else if (resolvedConfig.useOpenCodeFreeModels &&
        (resolvedConfig.availableOpenCodeFreeModels?.length ?? 0) > 0) {
        ...
        resolvedConfig.selectedOpenCodeSecondaryModel =
            resolvedConfig.selectedOpenCodeSecondaryModel ??
                pickSupportOpenCodeModel(availableModels, resolvedConfig.selectedOpenCodePrimaryModel)?.model ??
                resolvedConfig.selectedOpenCodePrimaryModel;
    ...
    if (resolvedConfig.hasChutes &&
        ...
        if (discovery.models.length === 0) {
            ...
            resolvedConfig.selectedChutesSecondaryModel =
                resolvedConfig.selectedChutesSecondaryModel ??
                    pickSupportChutesModel(discovery.models, resolvedConfig.selectedChutesPrimaryModel)?.model ??
                    resolvedConfig.selectedChutesPrimaryModel;
            printSuccess(`Chutes models ready (${discovery.models.length} models found)`);
        }
    }
    else if (resolvedConfig.hasChutes &&
        (resolvedConfig.availableChutesFreeModels?.length ?? 0) > 0) {
        ...
        resolvedConfig.selectedChutesSecondaryModel =
            resolvedConfig.selectedChutesSecondaryModel ??
                pickSupportChutesModel(availableChutes, resolvedConfig.selectedChutesPrimaryModel)?.model ??
                resolvedConfig.selectedChutesPrimaryModel;
    ...
    if (hasAnyEnabledProvider) {
        ...
        if (catalogDiscovery.models.length === 0) {
            printWarning(catalogDiscovery.error ??
...
export async function install(args) {
    ...
    if (!args.tui) {
        ...
        if (errors.length > 0) {
            ...
            console.log();
            printInfo('Usage: bunx miya install --no-tui --kimi=<yes|no> --openai=<yes|no> --anthropic=<yes|no> --copilot=<yes|no> --zai-plan=<yes|no> --antigravity=<yes|no> --chutes=<yes|no> --tmux=<yes|no>');
            console.log();
            return 1;
        }
