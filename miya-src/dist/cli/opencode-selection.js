import { pickBestModel, pickPrimaryAndSupport, } from './model-selection';
const scoreOpenCodePrimaryForCoding = (model) => {
    return ((model.reasoning ? 100 : 0) +
        (model.toolcall ? 80 : 0) +
...
const scoreOpenCodeSupportForCoding = (model) => {
    return ((model.toolcall ? 90 : 0) +
        (model.reasoning ? 50 : 0) +
        speedBonus(model.model) +
        Math.min(model.contextLimit, 400_000) / 20_000 +
        (model.status === 'active' ? 5 : 0));
};
export function pickBestCodingOpenCodeModel(models) {
    return pickBestModel(models, scoreOpenCodePrimaryForCoding);
}
export function pickSupportOpenCodeModel(models, primaryModel) {
