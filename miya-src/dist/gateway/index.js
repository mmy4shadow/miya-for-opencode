...
function buildStatusPayload(projectDir, runtime) {
    const kill = readKillSwitch(projectDir);
    return {
        updatedAt: new Date().toISOString(),
        autopilot: {
            status: 'placeholder',
            detail: 'Miya Autopilot control surface is online. Full orchestration metrics will be wired into this page incrementally.',
        },
        killSwitch: {
            active: kill.active,
...
function renderConsoleHtml(projectDir, runtime) {
...
export function ensureGatewayRunning(projectDir) {
...
function formatGatewayState(state) {
    return [
        `url=${state.url}`,
        `port=${state.port}`,
        `pid=${state.pid}`,
...
export function createGatewayTools(ctx) {
    const miya_gateway_start = tool({
        description: 'Start Miya Gateway (if needed) and persist .opencode/miya/gateway.json for Dock clients.',
        args: {},
        async execute() {
...
export function startGatewayWithLog(projectDir) {
    try {
