...
function formatJobs(jobs) {
    ...
    return jobs
        .map((job) => `- ${job.id} | ${job.name} | enabled=${job.enabled} | daily=${job.schedule.time} | next=${job.nextRunAt} | approval=${job.requireApproval}`)
        .join('\n');
}
    ...
    const miya_schedule_daily_command = tool({
        ...
        async execute(args) {
            const job = automationService.scheduleDailyCommand({
                ...
                requireApproval: typeof args.require_approval === 'boolean'
...
${result.stderr || '(empty)'}`;
    ...
    const miya_list_approvals = tool({
        ...
        async execute() {
            ...
            return approvals
                .map((approval) => `- ${approval.id} | job=${approval.jobId} | status=${approval.status} | requested=${approval.requestedAt}`)
                .join('\n');
        },
    });
...
timed_out=${result.result?.timedOut ?? 'n/a'}`;
    ...
    const miya_job_history = tool({
        ...
        async execute(args) {
            ...
            return records
                .map((record) => `- ${record.id} | job=${record.jobName} | trigger=${record.trigger} | status=${record.status} | exit=${record.exitCode} | started=${record.startedAt}`)
                .join('\n');
        },
    });
    const miya_status_panel = tool({
        ...
        async execute() {
            ...
            const historyText = history.length === 0
                ? '(no runs yet)'
                : history
                    .map((item) => `- ${item.startedAt} | ${item.jobName} | ${item.status} | exit=${item.exitCode}`)
                    .join('\n');
            const safety = getSafetySnapshot(automationService.getProjectDir());
            const safetyText = safety.recent.length === 0
                ? '(none)'
                : safety.recent
                    .map((item) => `- ${item.created_at} | ${item.status} | ${item.tier} | ${item.reason} | trace=${item.trace_id}`)
                    .join('\n');
...
${approvals.length === 0
                ? '(none)'
                : approvals
                    .map((item) => `- ${item.id} | job=${item.jobId} | requested=${item.requestedAt}`)
                    .join('\n')}
