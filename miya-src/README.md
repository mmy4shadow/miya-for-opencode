> This repository hosts Miya for OpenCode under `miya-src/` (source + built `dist/`).
...
```bash
bunx miya@latest install --no-tui --kimi=yes --openai=yes --antigravity=yes --chutes=yes --opencode-free=yes --opencode-free-model=auto --tmux=no --skills=yes
```
...
> **💡 Models are fully customizable.** Edit `~/.config/opencode/miya.json` (or `.jsonc` for comments support) to assign any model to any agent.
...
https://raw.githubusercontent.com/mmy4shadow/miya-for-opencode/refs/heads/main/miya-src/README.md
```

Note: this fork currently only includes the plugin source and build output; upstream docs/assets may not be present here.
