...
function writeWakeWords(
  projectDir: string,
  rows: Array<{ text: string; path?: string; weight?: number; tags?: string[] }>,
): void {
  const dir = path.join(getMiyaRuntimeDir(projectDir), 'model', 'sheng_yin', 'cache');
...
describe('audio filler adaptive cues', () => {
  test('uses adaptive wake-word cue when tag matches task kind', () => {
    ...
    const clipDir = path.join(getMiyaRuntimeDir(projectDir), 'model', 'sheng_yin', 'cache');
    fs.mkdirSync(clipDir, { recursive: true });
    fs.writeFileSync(path.join(clipDir, 'work.wav'), 'fake', 'utf-8');
    writeWakeWords(projectDir, [
      { text: '我在处理你的编码任务。', path: 'work.wav', weight: 5, tags: ['work'] },
      { text: '我在看图像。', weight: 1, tags: ['creative'] },
    ]);

    const controller = new AudioFillerController(projectDir, { random: () => 0 });
    ...
    expect(decision.cue?.clipPath?.endsWith(path.join('cache', 'work.wav'))).toBe(true);
