...
export function inferSentinelState(input?: SentinelSignals): SentinelInference {
  ...
  const looksLikeProtectedCapture = captureLimitations.some((item) =>
    ['drm', 'hdcp', 'protected', 'pmp', 'copyright'].some((flag) => item.includes(flag)),
  );
  const inputSignalConflict =
    ((idleSec ?? 0) >= 120 && (rawInputActive || switchRate >= 10)) ||
    (((foreground === 'game' || foreground === 'player') && !audioActive && (idleSec ?? 0) >= 120) &&
      switchRate >= 8);
  ...
  if (
    (foreground === 'ide' || foreground === 'terminal' || (foreground === 'browser' && apm >= 60)) &&
    (idleSec ?? 0) < 210 &&
    !fullscreen
  ) {
