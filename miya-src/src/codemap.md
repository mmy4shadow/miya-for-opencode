...
- Hooks are isolated (`hooks/loop-guard`, `phase-reminder`, `post-read-nudge`, `slash-command-bridge`) and exported via `hooks/index.ts`, so the plugin simply registers them via the `event`, `experimental.chat.messages.transform`, and `tool.execute.after` hooks defined in `index.ts`.
...
- Plugin registration: `index.ts` registers agents, the tool map (background/task, `grep`, `ast_grep_*`, `lsp_*`, `antigravity_quota`), MCP definitions (`createBuiltinMcps`), and hooks (`createLoopGuardHook`, `createPhaseReminderHook`, `createPostReadNudgeHook`, `createSlashCommandBridgeHook`); configuration hook merges those values back into the OpenCode config (default agent, permission rules parsed from `config/agent-mcps`, and MCP access policies).
...
- Hooks and helpers tie into external behavior: `hooks/loop-guard` and `hooks/phase-reminder/post-read-nudge/slash-command-bridge` enforce runtime guardrails and workflow reminders; `utils/logger.ts` centralizes structured logging used across modules.
