...
The `src/tools/` directory provides the core tool implementations for the miya plugin. It exposes three main categories of tools:
...
- **Install location**: `~/.cache/miya/bin/rg` (Linux/macOS), `%LOCALAPPDATA%\miya\bin\rg.exe` (Windows)
...
- **Install location**: `~/.cache/miya/bin/sg` (Linux/macOS), `%LOCALAPPDATA%\miya\bin\sg.exe` (Windows)
