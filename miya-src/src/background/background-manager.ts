...
export class BackgroundTaskManager {
  ...
  private getSubagentRules(agentName: string): readonly string[] {
    ...
    return (
      SUBAGENT_DELEGATION_RULES[
        normalizedAgentName as keyof typeof SUBAGENT_DELEGATION_RULES
      ] ?? ['2-code-search']
    );
  }
  ...
  isAgentAllowed(parentSessionId: string, requestedAgent: string): boolean {
    ...
    const parentAgentName =
      this.agentBySessionId.get(parentSessionId) ?? '1-task-manager';
  ...
  private resolveFallbackChain(agentName: string): string[] {
    ...
    const configuredChain =
      chains?.[agentName] ?? (legacyName ? chains?.[legacyName] : []) ?? [];
    const primary =
      this.config?.agents?.[agentName]?.model ??
