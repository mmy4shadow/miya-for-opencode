...
interface GatewayRequestIdempotencyRecord {
  key: string;
  requestHash: string;
  atMs: number;
  ok: boolean;
...
function buildGatewayChallengeSignature(input: {
  ...
  return createHmac('sha256', input.secret)
    .update(
      `${input.clientID}|${input.protocolVersion}|${input.nonce}|${String(input.ts)}`,
    )
    .digest('hex');
}
...
function normalizeIncomingProtocolVersion(input?: string): string {
  const raw = String(input ?? '').trim();
  if (!raw) return LEGACY_GATEWAY_PROTOCOL_VERSION;
  return raw;
}

function isSupportedProtocolVersion(version: string): boolean {
  return SUPPORTED_GATEWAY_PROTOCOL_VERSIONS.includes(
    version as (typeof SUPPORTED_GATEWAY_PROTOCOL_VERSIONS)[number],
  );
}
...
function readGatewayStateFile(projectDir: string): GatewayState | null {
  ...
  const uiUrl = String(raw.uiUrl ?? url).trim();
  const port = Number(raw.port);
  const pid = Number(raw.pid);
  const startedAt = String(raw.startedAt ?? '');
  const status = String(raw.status ?? 'running');
  const authTokenFromState =
    typeof raw.authToken === 'string' && raw.authToken.trim().length > 0
      ? raw.authToken.trim()
      : undefined;
...
function toGatewayState(
  ...
  const uiUrl = uiBasePath ? `${url}${uiBasePath}/` : url;
  return {
    url,
    uiUrl,
...
function collectDoctorIssues(
  ...
  if (
    ...
    issues.push({
      ...
      severity: 'warn',
      message:
        'Gateway is exposed on non-loopback host without challenge signature enforcement.',
...
function runGatewaySecurityAudit(
  projectDir: string,
  runtime: GatewayRuntime,
): {
  scannedAt: string;
  ...
  if (tokenSource === 'generated') {
    findings.push({
      ...
      severity: 'info',
      message:
        'Gateway token uses local generated secret (default safe baseline enabled).',
  ...
  if (
    ...
    findings.push({
      ...
      severity: 'warn',
      message:
        'Gateway bind host is non-loopback while challenge signature is disabled.',
  ...
  for (const channel of Object.values(channelStore.channels)) {
    ...
    if (channel.name === 'webchat') continue;
  ...
  if (findings.some((item) => item.severity === 'error')) status = 'error';
  else if (findings.some((item) => item.severity === 'warn')) status = 'warn';
...
function validateHelloChallenge(
  runtime: GatewayRuntime,
  ...
  if (!runtime.auth.challengeRequired) return { ok: true };
...
function idempotencyWindowMs(): number {
  return Math.max(
    ...
    Math.min(
      10 * 60_000,
      Number(process.env.MIYA_GATEWAY_IDEMPOTENCY_TTL_MS ?? 90_000),
    ),
  );
}

function cleanupIdempotency(runtime: GatewayRuntime): void {
  ...
  if (runtime.requestIdempotency.size <= maxEntries) return;
  const sorted = [...runtime.requestIdempotency.values()].sort(
    (a, b) => a.atMs - b.atMs,
  );
...
function requestFingerprint(frame: RequestFrame): string {
  return createHash('sha256')
    .update(`${frame.method}|${JSON.stringify(frame.params ?? {})}`)
    .digest('hex');
}
...
function createMethods(
  ...
  const callOpenClaw = async (
    ...
    if (!result.ok) {
      throw new Error(
        `openclaw_${result.error?.code ?? 'request_failed'}:${result.error?.message ?? 'request_failed'}`,
      );
    }
...
export function createGatewayTools(
  ...
  const miya_security_audit = tool({
    ...
    async execute() {
      ...
      ensureGatewayRunning(ctx.directory);
      const runtime = runtimes.get(ctx.directory);
      if (!runtime) return 'security_audit_failed=gateway_unavailable';
