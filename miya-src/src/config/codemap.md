...
- User config: `~/.config/opencode/miya.json` (or `$XDG_CONFIG_HOME`)
- Project config: `<directory>/.opencode/miya.json`
- Environment override: `MIYA_PRESET`
...
│
├─→ Load user config from ~/.config/opencode/miya.json
│   └─→ Validate with PluginConfigSchema
│       └─→ Return null if invalid/missing
│
├─→ Load project config from <directory>/.opencode/miya.json
│   └─→ Validate with PluginConfigSchema
│       └─→ Return null if invalid/missing
│
...
│
├─→ Apply environment preset override (MIYA_PRESET)
│
...
│
├─→ Check ~/.config/opencode/miya/{agentName}.md
│   └─→ If exists → read as replacement prompt
│
└─→ Check ~/.config/opencode/miya/{agentName}_append.md
