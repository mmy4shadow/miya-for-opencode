...
function getSavesDir(projectDir: string): string {
...
function savePath(projectDir: string, id: string): string {
  return path.join(getSavesDir(projectDir), `${id}.json`);
}

export function createSaveRecord(
  projectDir: string,
  input: Omit<MiyaSaveRecord, 'id' | 'createdAt' | 'branch'>,
): MiyaSaveRecord {
  const id = timestampId();
  const record: MiyaSaveRecord = {
    id,
    label: input.label,
...
export function loadSaveRecord(
  projectDir: string,
  id: string,
): MiyaSaveRecord | null {
  ...
  try {
    ...
    return JSON.parse(raw) as MiyaSaveRecord;
  } catch {
    return null;
  }
}

export function listSaveRecords(projectDir: string): MiyaSaveRecord[] {
  ...
  const records: MiyaSaveRecord[] = [];
  for (const fileName of files) {
    const fullPath = path.join(dir, fileName);
    try {
      const raw = fs.readFileSync(fullPath, 'utf-8');
      records.push(JSON.parse(raw) as MiyaSaveRecord);
    } catch {
...
export function evaluateSave(record: MiyaSaveRecord): {
