...
function normalizeAgentsRecord(
...
function normalizeFallbackChains(
...
function normalizeGeneratedConfig(config: Record<string, unknown>): void {
  ...
  const fallback = config.fallback as
    | { chains?: Record<string, string[]> }
    | undefined;
  if (fallback?.chains) {
...
export function generateAntigravityMixedPreset(
  ...
  const createAgentConfig = (
    ...
    if (
      (agentName === 'designer' || normalizedAgentName === '6-ui-designer') &&
    ...
    return {
      ...
      mcps:
        DEFAULT_AGENT_MCPS[
          normalizedAgentName as keyof typeof DEFAULT_AGENT_MCPS
        ] ?? [],
    };
  };
...
export function generateLiteConfig(
  ...
  if (installConfig.dynamicModelPlan) {
    ...
    (config.presets as Record<string, unknown>).dynamic =
