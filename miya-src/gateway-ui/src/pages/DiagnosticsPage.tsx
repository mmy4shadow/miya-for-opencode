...
const ConnectionStatusCard = React.memo<ConnectionStatusCardProps>(
  function ConnectionStatusCard({ gateway, daemon, statusError }) {
    const isConnected = gateway.status === 'online' && daemon.connected;
    const _hasError = gateway.status === 'error' || statusError;
      ...
      <Card title="连接状态" subtitle="网关与守门员连接">
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
            ...
            <div
              className={`font-bold ${daemon.connected ? 'text-green-600' : 'text-red-600'}`}
            >
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-lg ${node.connected ? 'text-green-600' : 'text-gray-400'}`}
                        >
                          {node.connected ? '🟢' : '⚪'}
                        </span>
