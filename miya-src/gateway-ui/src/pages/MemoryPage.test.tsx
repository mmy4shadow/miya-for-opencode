...
describe('Memory Module Regression Tests', () => {
  ...
  describe('Requirement 6.2: Memory Filtering', () => {
    it('should provide memory status filter options', async () => {
      ...
      expect(
        statusFilter.querySelector('option[value="all"]'),
      ).toBeInTheDocument();
      expect(
        statusFilter.querySelector('option[value="active"]'),
      ).toBeInTheDocument();
      expect(
        statusFilter.querySelector('option[value="pending"]'),
      ).toBeInTheDocument();
      expect(
        statusFilter.querySelector('option[value="superseded"]'),
      ).toBeInTheDocument();
      expect(
        statusFilter.querySelector('option[value="archived"]'),
      ).toBeInTheDocument();
    });
    ...
    it('should provide memory domain filter options', async () => {
      ...
      const domainFilter = screen.getByLabelText(
        '记忆域筛选',
      ) as HTMLSelectElement;

      // Verify domain filter options exist
      expect(
        domainFilter.querySelector('option[value="all"]'),
      ).toBeInTheDocument();
      expect(
        domainFilter.querySelector('option[value="work"]'),
      ).toBeInTheDocument();
      expect(
        domainFilter.querySelector('option[value="relationship"]'),
      ).toBeInTheDocument();
    });
