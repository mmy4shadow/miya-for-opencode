...
import type { GatewaySnapshot, PsycheModeConfig } from '../types/gateway';
...
describe('Property 6: Psyche Configuration Form Completeness', () => {
  ...
  it('should render form controls for all required configuration fields', () => {
    fc.assert(
      fc.property(psycheModeConfigArbitrary(), (config) => {
        ...
        const captureProbeCheckbox = container.querySelector(
          '#captureProbeEnabled',
        );
        ...
        const signalOverrideCheckbox = container.querySelector(
        ...
        const proactivitySlider = container.querySelector(
        ...
        const shadowCheckbox = container.querySelector(
        ...
        const shadowRolloutSlider = container.querySelector(
        ...
        const periodicRetrainCheckbox = container.querySelector(
        ...
        const proactivePingCheckbox = container.querySelector(
        ...
        const minIntervalInput = container.querySelector(
        ...
        const maxPerDayInput = container.querySelector(
        ...
        const quietHoursCheckbox =
          container.querySelector('#quietHoursEnabled');
        ...
        const timezoneOffset = container.querySelector(
          '#quietHoursTimezoneOffset',
        );
  ...
  it('should display configuration values correctly in form controls', () => {
    fc.assert(
      fc.property(psycheModeConfigArbitrary(), (config) => {
        ...
        const resonanceCheckbox = container.querySelector(
          '#resonanceEnabled',
        ) as HTMLInputElement;
        ...
        if (config.proactivityExploreRate !== undefined) {
          ...
          expect(parseInt(proactivitySlider?.value || '0', 10)).toBe(
            config.proactivityExploreRate,
          );
        }
        ...
        if (config.slowBrainShadowRollout !== undefined) {
          ...
          expect(parseInt(shadowRolloutSlider?.value || '0', 10)).toBe(
            config.slowBrainShadowRollout,
          );
        }
...
describe('Property 7: Psyche Configuration Save', () => {
  ...
  it('should have a save button that triggers save operation', () => {
    fc.assert(
      fc.property(psycheModeConfigArbitrary(), (config) => {
        ...
        ).find((btn) => btn.textContent?.includes('保存配置'));
  ...
  it('should call save handler when save button is clicked', async () => {
    await fc.assert(
      fc.asyncProperty(psycheModeConfigArbitrary(), async (config) => {
        const { container, unmount } = renderPsychePage(config);
        ...
        expect(
          buttonText?.includes('保存') || buttonText?.includes('保存中'),
        ).toBe(true);
    ...
    fc.assert(
      fc.property(
        ...
        (config) => {
          ...
          const shadowValue = parseInt(shadowRolloutSlider?.value || '0', 10);
