...
export function useMemoizedSnapshot(
  ...
  // Memoize jobs field
  const memoizedJobs = useMemo(() => snapshot.jobs, [stableDep(snapshot.jobs)]);
  ...
  const memoizedLoop = useMemo(() => snapshot.loop, [stableDep(snapshot.loop)]);
  const memoizedVoice = useMemo(() => snapshot.voice, [stableDep(snapshot.voice)]);
