...
export function GatewayProvider({
  ...
  const setKillSwitch = useCallback(
    async (mode: KillSwitchMode, reason?: string): Promise<void> => {
   ...
   * Update psyche mode configuration
   */
  const updatePsycheMode = useCallback(
    async (config: Partial<PsycheModeConfig>): Promise<void> => {
      if (!clientRef.current || actionInFlightRef.current) {
        return;
      }
  ...
  const updateTrustMode = useCallback(
    async (config: Partial<TrustModeConfig>): Promise<void> => {
      if (!clientRef.current || actionInFlightRef.current) {
        return;
      }
   ...
   * Toggle policy domain pause state
   */
  const togglePolicyDomain = useCallback(
    async (domain: string, paused: boolean): Promise<void> => {
      if (!clientRef.current || actionInFlightRef.current) {
        return;
      }
