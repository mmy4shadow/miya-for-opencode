  ...
  const miya_schedule_daily_command = tool({
    ...
    args: {
      ...
      require_approval: z
        .boolean()
        .optional()
        .describe('Compatibility flag (ignored in full-autopilot mode)'),
    },
    async execute(args) {
...
${result.stderr || '(empty)'}`;
  ...
  const miya_list_approvals = tool({
    description: 'List manual approvals (always empty in full-autopilot mode).',
    args: {},
    async execute() {
      return 'Manual approvals are disabled. Miya runs with self-approval evidence logging.';
    },
  });

  const miya_approve_job_run = tool({
    description: 'Compatibility endpoint. Manual approvals are disabled in full-autopilot mode.',
    args: {
      approval_id: z.string().describe('Approval request id'),
    },
    async execute(args) {
      void args;
      return 'Manual approvals are disabled. Use miya_run_job_now or scheduler execution.';
    },
  });
  ...
  const miya_reject_job_run = tool({
    ...
    async execute(args) {
      void args;
      return 'Manual approvals are disabled. Nothing to reject.';
    },
  });
  ...
  const miya_status_panel = tool({
    description: 'Show compact Miya runtime status panel for jobs and recent runs.',
    args: {},
    async execute() {
...
Pending approvals:
manual approvals are disabled (self-approval mode).
