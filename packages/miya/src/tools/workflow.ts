...
export function createWorkflowTools(
  ...
  const sid = (ctx: unknown) => {
    if (ctx && typeof ctx === 'object' && 'sessionID' in ctx) return String((ctx as { sessionID: string }).sessionID);
    return 'unknown';
  };
  ...
  const miya_iteration_done = tool({
    ...
    async execute(args, toolContext) {
      ...
      const done = Array.isArray(args.done) ? args.done.map(String) : [];
      const missing = Array.isArray(args.missing) ? args.missing.map(String) : [];
      const unresolved = Array.isArray(args.unresolved) ? args.unresolved.map(String) : [];
