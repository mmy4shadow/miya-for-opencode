...
const PROMPT_DIR_CANDIDATES = ['miya'] as const;
const CONFIG_BASE_CANDIDATES = ['miya'] as const;

function migrateLegacyConfigNames(directory: string): void {
  const userOpencodeDir = path.join(getUserConfigDir(), 'opencode');
  const projectOpencodeDir = path.join(directory, '.opencode');

  const migrations: Array<{ from: string; to: string }> = [
    {
      from: path.join(userOpencodeDir, 'oh-my-opencode-slim.json'),
      to: path.join(userOpencodeDir, 'miya.json'),
    },
    {
      from: path.join(userOpencodeDir, 'oh-my-opencode-slim.jsonc'),
      to: path.join(userOpencodeDir, 'miya.jsonc'),
    },
    {
      from: path.join(projectOpencodeDir, 'oh-my-opencode-slim.json'),
      to: path.join(projectOpencodeDir, 'miya.json'),
    },
    {
      from: path.join(projectOpencodeDir, 'oh-my-opencode-slim.jsonc'),
      to: path.join(projectOpencodeDir, 'miya.jsonc'),
    },
    {
      from: path.join(userOpencodeDir, 'oh-my-opencode-slim'),
      to: path.join(userOpencodeDir, 'miya'),
    },
  ];
  ...
  for (const migration of migrations) {
    try {
      ...
      if (fs.existsSync(migration.to)) {
        const fromStat = fs.statSync(migration.from);
        if (fromStat.isDirectory()) {
          fs.rmSync(migration.from, { recursive: true, force: true });
        } else {
      ...
      console.warn(
        `[miya] Failed to migrate ${migration.from} -> ${migration.to}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
...
export function loadPluginConfig(directory: string): PluginConfig {
  migrateLegacyConfigNames(directory);

  const userConfigPaths = CONFIG_BASE_CANDIDATES.map((name) =>
    path.join(getUserConfigDir(), 'opencode', name),
  );

  const projectConfigPaths = CONFIG_BASE_CANDIDATES.map((name) =>
    path.join(directory, '.opencode', name),
  );

  // Find existing config files (preferring candidates order + .jsonc over .json)
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[miya] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
...
export function loadAgentPrompt(agentName: string): {
  ...
  const promptsRoot = path.join(getUserConfigDir(), 'opencode');
  const result: { prompt?: string; appendPrompt?: string } = {};

  const promptsDir = PROMPT_DIR_CANDIDATES.map((name) =>
